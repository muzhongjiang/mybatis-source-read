...
/**
 * 默认SqlSession实现,该class 不是线程安全的（只能每次new一个对象）。
 *
...
public class DefaultSqlSession implements SqlSession {
    ...
    public <T> T selectOne(String statement, Object parameter) {
        /*
         内部去调用selectList,很简单的，如果得到0条则返回null，得到1条则返回1条，得到多条报TooManyResultsException错
        ...
        */
        ...
        if (list.size() == 1) {
            return list.get(0);
        } else if (list.size() > 1) {//得到多条报TooManyResultsException错
    ...
    public <K, V> Map<K, V> selectMap(String statement, Object parameter, String mapKey, RowBounds rowBounds) {
        ...
        //注意这个DefaultMapResultHandler里面存了所有已处理的记录(内部实现就是一个Map)，最后再返回一个Map ：
     ...
     */
    @Deprecated //严格的Map，如果找不到对应的key，直接抛BindingException例外，而不是返回null
