 ...
 * 要使用反射模块操作一个 Class，都会先将该 Class 封装成一个 Reflector 对象，
 * 在 Reflector 中缓存 Class 的元数据信息，这可以提高反射执行的效率。
 * <p>
 *  属性 -> getter/setter的映射器，而且加了缓存，可参考ReflectorTest来理解这个类的用处
 * <p>
...
public class Reflector {
    ...
    private final Map<String, Invoker> setMethods = new HashMap<>();//可写属性对应的setter 方法集合，key 是属性的名称，value 是一个 Invoker 对象。Invoker 是对 Method 对象的封装。
    private final Map<String, Invoker> getMethods = new HashMap<>();//可读属性对应的 getter 方法集合，key 是属性的名称，value 是一个 Invoker 对象。Invoker 是对 Method 对象的封装。
