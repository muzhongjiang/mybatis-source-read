...
class SqlRunnerTest extends BaseDataTest {
  ...
  @Test
  void shouldSelectOne1() throws Exception {
...
//    runScript(ds, JPETSTORE_DATA);
    try (Connection connection = ds.getConnection()) {
      ...
      String sql="select   if(status is null,'未知',status)   as status  FROM   edata_job_query_history where create_time >= DATE_ADD(now() ,INTERVAL -30 DAY) limit 1";
//      String sql="select  *    FROM   edata_job_query_history where create_time >= DATE_SUB(now() ,INTERVAL 30 DAY) limit 1";
      Map<String, Object> row = exec.selectOne(sql);
